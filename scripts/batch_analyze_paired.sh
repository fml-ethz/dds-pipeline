...
for dir in "$1"/*/; do
    ...
    # execute the analyze_paired.sh script
    /scripts/analyze_paired.sh "$dir/R1.fq.gz" "$dir/R2.fq.gz" "$dir/design_files.fasta" "${@:2}"
    ...
    if [ $? -eq 0 ]; then 
        ...
        echo "$dir_name: successfully processed" >> "$1/log_batch_analyze_paired.txt"
    else 
        printf "\n\n########################################################\n" "$dir"
        ...
        echo "$dir_name: error during processing, check logs" >> "$1/log_batch_analyze_paired.txt"
    fi
    ...
    for dir in 'fw' 'rv'; do
        for file in 'errorrates' 'readstats' 'delbias' 'insbias' 'subbias' 'delposition' 'insposition' 'subposition'; do
            /scripts/combine_data.sh "$dir/analysis_paired/${file}_${dir}.txt" "$dir_name" "$1/${file}_${dir}_batch_analyze_paired.txt"
        done
    done
done
