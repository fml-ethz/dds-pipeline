...
for dir in "$1"/*/; do
    ...
    # execute the analyze_single.sh script
    /scripts/analyze_single.sh "$dir/R1.fq.gz" "$dir/design_files.fasta" "${@:2}"
    ...
    if [ $? -eq 0 ]; then 
        ...
        echo "$dir_name: successfully processed" >> "$1/log_batch_analyze_single.txt"
    else 
        printf "\n\n########################################################\n" "$dir"
        ...
        echo "$dir_name: error during processing, check logs" >> "$1/log_batch_analyze_single.txt"
    fi
    ...
    for file in 'errorrates' 'readstats' 'delbias' 'insbias' 'subbias' 'delposition' 'insposition' 'subposition'; do
        /scripts/combine_data.sh "$dir/analysis_single/${file}_fw.txt" "$dir_name" "$1/${file}_fw_batch_analyze_single.txt"
    done
done
